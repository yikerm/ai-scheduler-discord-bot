...
class SegmentFeedbackTests(unittest.TestCase):
    ...
    def test_each_segment_uses_latest_calendar_time_and_updates_parent(self):
        ...
        with SessionLocal() as session:
            ...
            self.assertEqual(feedback.task_name_key, "分段報告")
            self.assertEqual(feedback.task_category, "writing")
            self.assertEqual(feedback.parent_total_minutes, 120)
            self.assertEqual(feedback.segment_index, 1)
            self.assertEqual(feedback.segment_count, 2)
            self.assertEqual(feedback.break_before_minutes, 0)
            self.assertEqual(feedback.prior_task_minutes, 0)
    ...
    def test_incomplete_segment_keeps_other_segment_and_marks_partial_at_end(self):
        ...
        with SessionLocal() as session:
            ...
            second_feedback = session.scalar(
                session.query(Feedback)
                .filter(Feedback.segment_id == second_id)
                .statement
            )
            self.assertEqual(second_feedback.segment_index, 2)
            self.assertEqual(second_feedback.segment_count, 2)
            self.assertEqual(second_feedback.break_before_minutes, 120)
            self.assertEqual(second_feedback.prior_task_minutes, 60)
            self.assertEqual(
                session.query(Feedback)
